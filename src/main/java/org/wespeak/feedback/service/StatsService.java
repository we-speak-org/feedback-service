...
public class StatsService {
    ...
    public ProgressHistoryResponse getMyHistory(String userId, String languageCode, String period) {
        ...
        Map<LocalDate, List<Feedback>> feedbacksByDate = feedbacks.stream()
                .filter(f -> f.getCreatedAt().isAfter(cutoffDate))
                .collect(Collectors.groupingBy(f -> 
                        LocalDate.ofInstant(f.getCreatedAt(), ZoneId.systemDefault())));
        
        List<ProgressDataPoint> dataPoints = feedbacksByDate.entrySet().stream()
                .map(entry -> {
                    LocalDate date = entry.getKey();
                    List<Feedback> dayFeedbacks = entry.getValue();
                    
                    int avgScore = (int) dayFeedbacks.stream()
    ...
    private Instant calculateCutoffDate(String period) {
        ...
        return switch (period != null ? period.toUpperCase() : "MONTH") {
            ...
            case "MONTH" -> now.minus(30, ChronoUnit.DAYS).atStartOfDay(ZoneId.systemDefault()).toInstant();
